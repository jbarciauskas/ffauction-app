...
class App extends React.Component {
  ...
  resetKeepers(event) {
    var doubleCheck = confirm("Are you sure you want to clear price and team data for keepers?");
    if(doubleCheck) {
      this.state.rowData.forEach((player) => {
        if(player.keeper == 'Yes') {
  ...
  restartAuction(event) {
    ...
    if(doubleCheck) {
      this.state.rowData.forEach((player) => {
        if(player.purchase_price != 0 && player.keeper != 'Yes') {
          player.purchase_price = 0;
          player.draft_team = null;
          player.keeper = 'No';
          localStorage.removeItem("player-" + player.player_id);
        ...
        <Grid>
          <Navbar inverse>
            ...
            <Nav>
              ...
              <NavItem eventKey={1} href="#" onClick={this.restartAuction}>Restart auction</NavItem>
              <NavItem eventKey={1} href="#" onClick={this.resetKeepers}>Reset keepers</NavItem>
            </Nav>
          </Navbar>
          <Row>
            <Col md={12}>
